...
function getSellRation(total_amt, sell_amt){
  if(total_amt && sell_amt){
    return FormatRate( parseFloat(sell_amt / total_amt).toFixed(2));
  }else if(sell_amt === 0){
    return '0%';
  }else{
    return '-';
...
function getRestAmt(total_amt,sell_amt) {
  if(total_amt ){
    return (total_amt - (sell_amt ? sell_amt : '0'));
  }else{
    return '-';
  }
}
...
const OnlinePlans = ({state, onSearch, onChange, onMenuClick, onMenuToggleClick, onRefresh, onPage, onFocusChange, onDatesChange,onStatusChange}) => {
  ...
  initData['rows'] = (data.list && data.list.map(item => {
      ...
      let detail_url = "/plans/detail/" + item['id'];
      return [
        item['id'],
        item['product_title'],
        ...
        getSellRation(item['total_amt'],item['sell_amt']),
        getRestAmt(item['total_amt'],item['sell_amt']),
  ...
  return (
    <div className="row" style={{minHeight: '100%'}}>
      <div className="col-12" style={{position: 'relative'}}>
        ...
        <Card
          header={{
            text: <span className="font-green-sharp bold"><i className="fa fa-users"/> 已上线理财计划 </span>
          }}
